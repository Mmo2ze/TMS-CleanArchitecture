...
public class CreateAutoSchedulerHandler : IRequestHandler<CreateAutoSchedulerCommand,
    ErrorOr<List<Domain.AttendanceSchedulers.AttendanceScheduler>>>
{
    ...
    public CreateAutoSchedulerHandler(ITeacherHelper teacherHelper,
        IAttendanceSchedulerRepository attendanceSchedulerRepository, ITeacherRepository teacherRepository,
    ...
    public async Task<ErrorOr<List<Domain.AttendanceSchedulers.AttendanceScheduler>>> Handle(
        CreateAutoSchedulerCommand request, CancellationToken cancellationToken)
    {
        ...
        List<Domain.AttendanceSchedulers.AttendanceScheduler> newSchedulers = [];
        ...
        if (request.SchedulerOption == AutoAttendanceSchedulerOption.AfterEverySession)
        {
            ...
            {
                newSchedulers.Add(
                    Domain.AttendanceSchedulers.AttendanceScheduler.Create(session.Day, session.EndTime, teacherId));
            }
        }
        else if (request.SchedulerOption == AutoAttendanceSchedulerOption.AfterLastSessionOfSameGrade)
        {
            ...
            foreach (var group in sessionsGroupedByGrade)
            {
                ...
                newSchedulers.Add(
                    Domain.AttendanceSchedulers.AttendanceScheduler.Create(lastSession.Day, lastSession.EndTime,
